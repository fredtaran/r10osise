...
const checkUsername = (username) => {
    ...
    axios.get(`check_username?${queryString}`);
}
                    ...
                    <form @submit.prevent="submitForm">
                        <div class="grid gap-6">
                            <div>
                                ...
                                <span class="text-sm text-red-700 font-italic font-bold" v-for="error in v$.username.$errors" :key="error.uid">{{ error.$message }}</span>
                            </div>
                            ...
                            <div>
                                ...
                                <span class="text-sm text-red-700 font-italic font-bold" v-for="error in v$.password.$errors" :key="error.uid">{{ error.$message }}</span>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="password" id="confirm_password" autocomplete="off"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
                                    v-model="formData.confirm_password" required>
                                <span class="text-sm text-red-700 font-italic font-bold" v-for="error in v$.confirm_password.$errors" :key="error.uid">{{ error.$message
                                }}</span>
                            </div>
                            ...
                            <span>Already have an account? Login <RouterLink :to="{ 'name': 'login' }" class="underline text-blue-600 font-bold hover:text-red-600">here</RouterLink>.</span>
                        </div>
                    </form>
                </div>
            </div>
